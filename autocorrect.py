...
class Autocorrect:
    ...
    def plot_correction_statistics(self):
        """Create an enhanced visualization of correction statistics."""
        plt.figure(figsize=(12, 7), facecolor='#f0f0f0')

        # Prepare data
        words = list(self.correction_stats.keys())
        frequencies = list(self.correction_stats.values())

        # Create a styled bar plot with seaborn
        plt.style.use('seaborn')
        ax = sns.barplot(
            x=words,
            y=frequencies,
            palette='cool',  # Use a gradient color palette
            edgecolor='black',
            linewidth=1.2
        )

        # Customize the plot
        plt.title('Spelling Correction Frequency', fontsize=16, fontweight='bold')
        plt.xlabel('Corrected Words', fontsize=12)
        plt.ylabel('Number of Corrections', fontsize=12)
        ...
        plt.text(
            0.95,
            0.95,
            f'Total Corrections: {sum(frequencies)}',
